<?php
	...
	class user {
		...
		function apiKey($odb) {
			if ($this->hasMembership($odb)) {
				$SQL = $odb -> prepare("SELECT `api` FROM `users` WHERE `ID` = ?");
				$SQL -> execute(array($_SESSION['ID']));
				$key = $SQL -> fetchColumn(0);
				return $key;
			}
			return '-1';
		}
		...
		function hasMembershipApi($odb, $id){
			$SQL = $odb -> prepare("SELECT `expire` FROM `users` WHERE `ID` = ?");
	...
	class stats {
		...
		function concurrentsApi($odb, $id){
			$SQL = $odb -> prepare("SELECT `plans`.`concurrents` FROM `plans` LEFT JOIN `users` ON `users`.`membership` = `plans`.`ID` WHERE `users`.`ID` = ?");
			$SQL -> execute(array($id));
			return $SQL->fetchColumn(0);
		}
